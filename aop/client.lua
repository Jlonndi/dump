...
AddEventHandler('playerSpawned', function()
    if AOPSpawnsEnabled then
        TriggerEvent('AOP:SetPlayerSpawnPoint', PlayerPedId())
    end
end)
...
Citizen.CreateThread(function()
    while true do
        if localTime == 1 then -- client time
            ...
            drawTimeText = featColor .. "~y~Time: ~w~" .. hour .. ":" .. newMinute .. featColor .." ~y~| Date: ~w~" .. day .. featColor .."~y~/~w~" .. month .. featColor .. "~y~/~w~" .. year
        ...
        if peacetimeActive then
            ...
            DrawTextAOP(AOPxNew, AOPyNew2, 1.0,1.0,0.45, "~w~Current " .. featColor .. "~y~AOP: ~w~" .. FaxCurAOP .. featColor .. " ~y~| ~w~PeaceTime: ~g~Enabled", 255, 255, 255, 255)
        elseif not peacetimeActive then
            if peacetime then
                DrawTextAOP(AOPxNew, AOPyNew, 1.0,1.0,0.45, drawTimeText, 255, 255, 255, 255)
                DrawTextAOP(AOPxNew, AOPyNew2, 1.0,1.0,0.45, "~w~Current " .. featColor .. "~y~AOP: ~w~" .. FaxCurAOP .. featColor .. " ~y~| ~w~PeaceTime: ~r~Disabled", 255, 255, 255, 255)
            else
                DrawTextAOP(AOPxNew, AOPyNew, 1.0,1.0,0.45, drawTimeText, 255, 255, 255, 255)
                DrawTextAOP(AOPxNew, AOPyNew2, 1.0,1.0,0.45, "~w~Current " .. featColor .. "~y~AOP: ~w~" .. FaxCurAOP, 255, 255, 255, 255)
            end
        end
	end
end)

...
AddEventHandler("izone:isPlayerInZone", function(zone, cb)
	...
	if not found then
		...
		if IsEntityAtCoord(PlayerPedId(), tonumber(allZone[found].gravityCenter.x), tonumber(allZone[found].gravityCenter.y), tonumber(allZone[found].longestDistance), tonumber(allZone[found].longestDistance), tonumber(allZone[found].longestDistance), 0, 1, 0) then
...
RegisterNetEvent("izone:isPlayerInAnyZoneMatching")
AddEventHandler("izone:isPlayerInAnyZoneMatching", function(search, callback)
	local arrayReturn = {}
	local plyCoords = GetEntityCoords(PlayerPedId())
	local x1, y1, z1 = table.unpack(plyCoords)
	...
	for i=1, #allZone do
		if GetDistanceBetweenCoords(x1, y1, z1, tonumber(allZone[i].gravityCenter.x), tonumber(allZone[i].gravityCenter.y), 1.01, false) < tonumber(allZone[i].longestDistance) then
			local n = windPnPoly(allZone[i].coords, plyCoords)
			if n ~= 0 and string.match(allZone[i].name, search) then
				table.insert(arrayReturn, allZone[i].name)
				print("matched " .. allZone[i].name)
			end
		end
	end
...
function TeleportPlayerToCoords(x, y, z)
	local myPly = PlayerPedId()
	SetEntityCoords(myPly, tonumber(x), tonumber(y), tonumber(z), true, true, true, false)
end

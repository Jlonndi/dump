				...
				if(veh ~= nil and GetVehicleNumberPlateText(veh) ~= nil) then
					...
					--if(number == 883325847) then
						...
						if(IsControlJustPressed(1, 38)) then
							...
							print('registering jerry can finnisheds')
							TriggerServerEvent('blrp_inventory:registerFiredBullet', 'WEAPON_PETROLCAN')
							TriggerServerEvent('blrp_inventory:saveUsedBullets', 'WEAPON_PETROLCAN')
...
Citizen.CreateThread(function()
	...
	while true do
		...
		--if IsPedInAnyVehicle(PlayerPedId(), -1) then
			...
			if(isNearFuelStation) then-- and IsPedInAnyVehicle(PlayerPedId(), -1) and not IsPedInAnyHeli(PlayerPedId()) and not isBlackListedModel() and not isElectricModel() and GetPedVehicleSeat(PlayerPedId()) == -1) then
		    ...
		    if veh ~= nil and IsEntityAVehicle(veh) then
					...
					--		Info(settings[lang].ecxclectricError)
					--	end
				else
					if not IsPedInAnyVehicle(PlayerPedId(), -1) then
						Info('Petrol cans can be crafted with oil')
					end
				end
			end
			...
			if(isNearElectricStation() and IsPedInAnyVehicle(PlayerPedId(), -1) and not IsPedInAnyHeli(PlayerPedId()) and not isBlackListedModel() and isElectricModel() and GetPedVehicleSeat(PlayerPedId()) == -1) then
				Info(settings[lang].openMenu)

				if(IsControlJustPressed(1, 38)) and not GetIsVehicleEngineRunning(GetVehiclePedIsIn(PlayerPedId(), true))  then
					menu = not menu
					int = 0
					--[[Menu.hidden = not Menu.hidden
			...
			if(isNearFuelBStation and IsPedInAnyVehicle(PlayerPedId(), -1) and not IsPedInAnyHeli(PlayerPedId()) and not isBlackListedModel() and GetPedVehicleSeat(PlayerPedId()) == -1) then
				Info(settings[lang].openMenu)
			...
			if(isNearFuelPStation and IsPedInAnyVehicle(PlayerPedId(), -1) and not isBlackListedModel() and IsPedInAnyPlane(PlayerPedId()) and GetPedVehicleSeat(PlayerPedId()) == -1) then
				Info(settings[lang].openMenu)
			...
			if(isNearFuelHStation and IsPedInAnyVehicle(PlayerPedId(), -1) and not isBlackListedModel() and IsPedInAnyHeli(PlayerPedId()) and GetPedVehicleSeat(PlayerPedId()) == -1) then
				Info(settings[lang].openMenu)
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(1000)
		--local isEngineOn = Citizen.InvokeNative(0xAE31E7DF9B5B132E, GetVehiclePedIsIn(PlayerPedId())) -- Thanks to Asser
		if(IsPedInAnyVehicle(PlayerPedId(), -1) and GetIsVehicleEngineRunning(GetVehiclePedIsIn(PlayerPedId())) and GetPedVehicleSeat(PlayerPedId()) == -1 and not isBlackListedModel()) then
			local mph = GetEntitySpeed(GetVehiclePedIsIn(PlayerPedId(), false)) * 2.236936
			local vitesse = math.ceil(mph)
			local hasTurbo = GetVehicleMod(GetVehiclePedIsIn(PlayerPedId()),18)
...
function renderBoxes()
	if(IsPedInAnyVehicle(PlayerPedId(), -1) and GetPedVehicleSeat(PlayerPedId()) == -1 ) then
		if showUI then
			local percent = (essence/0.142)*100
...
AddEventHandler('essence:syncWithAllPlayers', function(fuel, vplate, vmodel)
	...
	if(IsPedInAnyVehicle(PlayerPedId())) then
		currentPedVModel = GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsUsing(PlayerPedId())))
		currentPedVPlate = GetVehicleNumberPlateText(GetVehiclePedIsUsing(PlayerPedId()))
...
AddEventHandler("essence:hasBuying", function(amount)
	if not refuelInProgress then
		...
		while done == false do
			...
			local vehClass = GetVehicleClass(GetVehiclePedIsIn(PlayerPedId(),false))
			if not GetIsVehicleEngineRunning(vechileForFuel) and (not IsPedInAnyVehicle(PlayerPedId(), -1) or (vehClass == 14 or vehClass == 15 or vehClass == 16)) then
				if(amountToEssence-0.0005 > 0) then
					amountToEssence = amountToEssence-0.0005
					essence = _essence + 0.0005
					_essence = essence

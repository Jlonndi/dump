...
function InputElement(_label, _type, _value, _can_be_empty, _can_be_edited, _disabled)
{
	this.label 			= _label;
	this.type 			= _type;
	this.value 			= _value;
	...
	this.getHTML 	= function(_editable) {
		...
		let _dtext = ''
		if(this.disabled) _dtext = 'disabled'
		...
		if (this.type === "textarea") {
			...
			_html += `<textarea ${_dtext} id="${this.elementid}">${this.value}</textarea>`;
		}

		if (this.type === "input") {
			_html += `<p class="input_label">${this.label}</p>`;
			_html += `<input type="text" id="${this.elementid}" value="${this.value}" ${_dtext}/>`;
		}
...
function Form (_title, _subtitle, _elements, _submittable)
{
	...
	this.loadFromJson = function(obj) {
		...
		{
			let tmpIE = new InputElement(obj.elements[i].label, obj.elements[i].type, obj.elements[i].value, obj.elements[i].can_be_empty, obj.elements[i].can_be_edited, obj.elements[i].disabled)
	...
	this.submit = function() { 
		...
		activeform.id = current_id;
		activeform.phone = current_id;
		var json_string = JSON.stringify(activeform);
		
		if (can_submit) {
			$.post('http://blrp_documents/form_submit', json_string);
			activeform.close();
		}
	...
	this.print = function () { 
		...
		if (this.submittable) {
			...
			$('#section_block').text('FIELDS')
			if (!this.signed) {
				setTimeout(() => {
					// $('#header_details').hide()
					$('#section_footer_block').hide()
					$('#section_footer').hide()
					$('#header_information_block').text('SENSITIVE')
					$('.header_information_subblock').hide()
				}, 1)
			}
		}
	...
	this.close = function() {
		...
		$.post('http://blrp_documents/form_close', JSON.stringify({}));
	}
}
...
window.addEventListener('message', function(event){
   	...
   	if (edata.type == "createNewForm") {
   		activeform = new Form();
   		activeform.loadFromJson(edata.data);
   		current_id = edata.data.id
   		current_phone = edata.data.phone

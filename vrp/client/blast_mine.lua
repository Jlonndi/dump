...
local vein_timer = 90
local loot_table = {
	[1] = {"Raw Aluminum","raw_aluminum",1}, -- 10
	[2] = {"Raw Steel","raw_steel",2}, -- 5
	[3] = {"Raw Titanium","raw_titanium",4}, -- 2
}
...
local function roll_loot()
	...
	if value > 2 then
		...
		-- 10% chance to get platinum
		if math.random(0, 100) <= 10 then
			total_loot[4] = {
				['loot_string'] = 'Raw Platinum',
				['loot_item'] = 'raw_platinum',
				['loot_count'] = 1
			}
		end
	end
...
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(1000)
		for _,site in pairs(scavenger_sites) do
			for k,v in pairs(site.loot_areas) do
...
-- This thread deals with keypress monitoring
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(0)
...
Citizen.CreateThread(function()
	while true do
		...
		for _,site in pairs(scavenger_sites) do
			if #(vector3(pos.x, pos.y, pos.z) - vector3(site.x, site.y, site.z)) < 3.0 then
			...
			if(clocked_in) then
				for k,v in pairs(site.loot_areas) do
					if not IsPedInAnyVehicle(PlayerPedId(), false) then
						local distance = #(vector3(pos.x, pos.y, pos.z) - vector3(v.x, v.y, v.z))
				...
				if #(vector3(pos.x, pos.y, pos.z) - vector3(site.x, site.y, site.z)) > 300.0 then
					clocked_in = false
...
Citizen.CreateThread(function()
	for k,v in pairs(scavenger_sites) do
		if v.blip then

...
function tvRP.setMedic(flag)
  ...
  if medic then
    ...
    --SetPedRelationshipGroupHash(PlayerPedId(), GetHashKey("blrp_ems")) --add player to non-agro group
    ...
    RemoveWeaponFromPed(PlayerPedId(),0xCB13D282) -- WEAPON_FIREEXTINGUISHER (pickup)
    RemoveWeaponFromPed(PlayerPedId(),0x060EC506) -- WEAPON_FIREEXTINGUISHER
    vRPserver.removePlayerToActiveEMS({})
    SetPedRelationshipGroupHash(PlayerPedId(), GetHashKey("PLAYER")) --set player back to default group
	...
	Citizen.CreateThread(function()
		while medic do
			...
			if nearServId ~= -1 and not IsPedInAnyVehicle(ped, true) then
				...
				if target ~= 0 and IsEntityAPed(target) and (IsEntityPlayingAnim(target,"missprologueig_6","lying_dead_brad",3) or IsEntityPlayingAnim(target,"missprologueig_6","lying_dead_player0",3) or IsEntityPlayingAnim(target,"missfinale_c1@","lying_dead_player0",3)) and not IsPedInAnyVehicle(target, true) then
  ...
  while treating do
    ...
    local x,y,z = table.unpack(GetEntityCoords(PlayerPedId(),true))
		...
		if nearPlayer ~= -1 then
			local targetPed = PlayerPedId(GetPlayerFromServerId(nearServId))

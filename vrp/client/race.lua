...
function tvRP.startRace(raceID,rCoordx,rCoordy,rCoordz)
  if not inRace then
    ...
    local startposX,startposY,startposZ = table.unpack(GetEntityCoords(PlayerPedId()))
    while preRace do
      Citizen.Wait(0)
      curX,curY,curZ = table.unpack(GetEntityCoords(PlayerPedId()))
    ...
    if not falseStart then
      ...
      Citizen.CreateThread(function()
      	while inRace do
      		Citizen.Wait(0)
          if IsEntityAtCoord(PlayerPedId(), rCoordx, rCoordy, rCoordz, 7.001, 7.001, 7.001, 0, 1, 0) then

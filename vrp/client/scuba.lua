	...
	while true do
		...
		if(scuba and scubaTimer > 0) then
			if(tvRP.isInWater()) then
				if not inWater then
					tvRP.attachProp('p_s_scuba_tank_s',24818,-0.290001,-0.240001,0,180.000001,270.000001,0,true)
					tvRP.attachProp('p_d_scuba_mask_s',31086,0,0,0,180.000001,270.000001,0,true)
					inWater = true
				end
...
AddEventHandler("scuba:createBlip", function(type, x, y, z, name)
	...
	AddTextComponentString("Underwater Wreck")

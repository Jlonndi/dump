...
local currentlyTowedVehicleNetID = nil
...
function tvRP.getIsCurrentlyTowing()
    if currentlyTowedVehicleNetID ~= nil then
        return true
    end
    return false
end
...
AddEventHandler('tow', function()
    ...
    if isVehicleTow then
        ...
        local targetVehicle = -1
        tvRP.selectTargetVehicle(4, function(targetVehicle)
            if currentlyTowedVehicleNetID == nil then
                if targetVehicle ~= nil and targetVehicle ~= -1 then
                                ...
                                if vehicle ~= targetVehicle and IsVehicleStopped(vehicle) then
                                    exports['mythic_progbar']:Progress({
                                        ...
                                        if not status then
                                            ...
                                            currentlyTowedVehicleNetID = NetworkGetNetworkIdFromEntity(targetVehicle)
                    ...
                    Citizen.CreateThread(function()
                        while currentlyTowedVehicleNetID ~= nil do
                            Citizen.Wait(0)
                            isVehicleTow = tvRP.isVehicleATowTruck(vehicle)
                            local roll = GetEntityRoll(GetVehiclePedIsIn(PlayerPedId(), true))
                            if currentlyTowedVehicleNetID ~= nil and IsEntityUpsidedown(GetVehiclePedIsIn(PlayerPedId(), true)) and isVehicleTow or roll > 70.0 or roll < -70.0 then
                                currentlyTowedVehicle = NetworkGetEntityFromNetworkId(currentlyTowedVehicleNetID)
                                DetachEntity(currentlyTowedVehicle, false, false)
                                currentlyTowedVehicle = nil
                                currentlyTowedVehicleNetID = nil
            ...
            elseif IsVehicleStopped(vehicle) then
                exports['mythic_progbar']:Progress({
                    ...
                    if not status then
                        ...
                        SetEntityCoords(currentlyTowedVehicle, vehiclesCoords["x"], vehiclesCoords["y"], vehiclesCoords["z"], true, true, true, false)
...
function goToTowCarThread(x, y, z)
    ...
    Citizen.Wait(3000)
    local closestParkedVehicles = GetClosestParkedVehicles(300, 20)
    --print('closestParkedVehicles is', json.encode(closestParkedVehicles))
    if closestParkedVehicles ~= nil and #closestParkedVehicles > 0 then
        ...
        if closestVehicle ~= nil then
            ...
            tvRP.notify("A competing firm got the job (Close vehicle not found)")
        end
    else
        tvRP.notify("A competing firm got the job (No towable vehicles nearby")
    end
end

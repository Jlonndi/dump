...
-- PROMPT
client_only_prompt = false

RegisterNetEvent('vrp:client:promptCallback')
AddEventHandler('vrp:client:promptCallback', function(title, callback)
  client_only_prompt = callback
  tvRP.prompt(title, '')
end)

-- Do callback for entered text
function confirmClientPrompt(response)
  client_only_prompt(response)
  client_only_prompt = nil
end
...
function tvRP.requestRaceRange(id,text,time,coordx, coordy, coordz,range)
  if IsEntityAtCoord(PlayerPedId(), coordx, coordy, coordz, 15.0, 15.0, 10.0, 0, 1, 0) then
    if IsPedInAnyVehicle(PlayerPedId(), false) then
      local veh = GetVehiclePedIsIn(PlayerPedId(), false)
      if veh ~= nil and (GetPedInVehicleSeat(veh, -1) == PlayerPedId()) then
        SendNUIMessage({act="request",id=id,text=tostring(text),time = time})
        tvRP.playSound("HUD_MINI_GAME_SOUNDSET","5_SEC_WARNING")
      end
    end
...
RegisterNUICallback("prompt",function(data,cb)
  if data.act == "close" then
    SetNuiFocus(false)
    SetNuiFocus(false)
    if client_only_prompt then
      confirmClientPrompt(data.result)
    else
...
function enableAdminMenu()
  if tvRP.isAdmin() then
    RegisterCommand("open-menueh", function(source, args, rawCommand)
        if tvRP.isAdmin() and not menu_state.opened then
          vRPserver.openMainMenu({})
        end
    end, false)

    RegisterKeyMapping("open-menueh", "Shortcut to a menu", "keyboard", '\\')
  end
end
...
Citizen.CreateThread(function()
  while true do
    ...
    -- open general menu
    --if IsDisabledControlJustPressed(table.unpack(cfg.controls.phone.open)) and tvRP.isAdmin() and not menu_state.opened then vRPserver.openMainMenu({}) end

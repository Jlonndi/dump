...
AddEventHandler("mm:changeeverything_spawn",function(user)
    ...
    --SetPedComponentVariation(PlayerPedId(), 0, tonumber(user.head), 0, 0)
    --The laziest way possible of doing this ^_^
    SetPedHeadBlendData(PlayerPedId(), tonumber(c_options.head), tonumber(c_options.head), 0, tonumber(c_options.head), tonumber(c_options.head), 0, 0.5, 0.5, 0.0, false)
    SetPedComponentVariation(PlayerPedId(), 1, tonumber(user.mask), tonumber(user.mask_txt), 0)
    SetPedComponentVariation(PlayerPedId(), 2, tonumber(user.hair), 0, 1)
    SetPedHairColor(PlayerPedId(), tonumber(user.hcolour), tonumber(user.hcolour))
    SetPedComponentVariation(PlayerPedId(), 6, tonumber(user.shoe), tonumber(user.shoe_txt), 0)
    SetPedComponentVariation(PlayerPedId(), 11, 0, 240, 0)
    SetPedComponentVariation(PlayerPedId(), 8, 0, 240, 0)
    SetPedComponentVariation(PlayerPedId(), 11, tonumber(user.shirt), tonumber(user.shirt_txt), 0)
    SetPedComponentVariation(PlayerPedId(), 3, tonumber(user.hand), 0, 0)
    SetPedComponentVariation(PlayerPedId(), 4, tonumber(user.pants), tonumber(user.pants_txt), 0)
    SetPedComponentVariation(PlayerPedId(), 8, tonumber(user.undershirt), tonumber(user.undershirt_txt), 0)
    SetPedComponentVariation(PlayerPedId(), 9, tonumber(user.armour), tonumber(user.armour_txt), 0)
    SetPedPropIndex(PlayerPedId(), 0, tonumber(user.helmet), tonumber(user.helmet_txt), 0)
    SetPedPropIndex(PlayerPedId(), 1, tonumber(user.glasses), tonumber(user.glasses_txt), 0)
    SetPedPropIndex(PlayerPedId(), 2, tonumber(user.piercing), 0, 0)
end)
...
function changempmodel(model)
    ...
    if model == 'mp_f_freemode_01' then
        SetPedComponentVariation(PlayerPedId(), 0, 34, 0, 0)
    end
    SetModelAsNoLongerNeeded(modelhashed)
    tvRP.setHealth(currentHealth)
    mp_check = true
...
function checkCustomisation()
    ...
    if(GetEntityModel(PlayerPedId()) == hashMaleMPSkin) or (GetEntityModel(PlayerPedId()) == hashFemaleMPSkin) then
...
function Main()
    local myUserID = tvRP.getMyVrpId()
    ...
    if tvRP.isAdmin() then
      ...
      Menu.addButton("Admin Models", "AdminMenu", nil)
    end
    if tvRP.isAdmin() or special_access_users[myUserID] ~= nil then
      Menu.addButton("Custom Models", "CustomMenu", nil)
    end
...
function AdminMenu()
    DisplayHelpText("Use ~INPUT_CELLPHONE_UP~ ~INPUT_CELLPHONE_DOWN~ to ~y~move~w~ and ~y~Enter~w~ to ~r~select")
    options.menu_subtitle = "Admin Models"
    ClearMenu()
    Menu.previous = "Main"
    for i,c in pairs(admin_models) do
        Menu.addButton(admin_models[Menu.buttonCount+1], "savemodel", admin_models[Menu.buttonCount+1])
    end
end

function CustomMenu()
    DisplayHelpText("Use ~INPUT_CELLPHONE_UP~ ~INPUT_CELLPHONE_DOWN~ to ~y~move~w~ and ~y~Enter~w~ to ~r~select")
    options.menu_subtitle = "Custom Models"
    ClearMenu()
    Menu.previous = "Main"
    for i,c in pairs(custom_models) do
        Menu.addButton(custom_models[Menu.buttonCount+1], "savemodel", custom_models[Menu.buttonCount+1])
    end
end
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--Others Menu
...
function ripmask()
    c_options.mask = 0
    SetPedComponentVariation(PlayerPedId(), 1, tonumber(c_options.mask), 0, 0)
end
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--Mask Menu
--Masks
...
function savetxt(mask_txt) --Sets mask texture in database
    ...
    SetPedComponentVariation(PlayerPedId(), 1, tonumber(c_options.mask), tonumber(c_options.mask_txt), 0)
end
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
function sethelmet(helmet)
    ...
    SetPedPropIndex(PlayerPedId(), 0, tonumber(c_options.helmet), 0, 0)
    helmetTextures()
end
...
function sethelmet_txt(helmet_txt)
    c_options.helmet_txt = helmet_txt
    SetPedPropIndex(PlayerPedId(), 0, tonumber(c_options.helmet), tonumber(c_options.helmet_txt), 0)
end
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--Accessories
--Glasses Menu
...
function setglasses(glasses)
    ...
    SetPedPropIndex(PlayerPedId(), 1, tonumber(c_options.glasses), 0, 0)
    glassesTextures()
end
...
function setglasses_txt(glasses_txt)
   ...
   SetPedPropIndex(PlayerPedId(), 1, tonumber(c_options.glasses), tonumber(c_options.glasses_txt), 0)
end
--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--Accessories
--Piercing Menu
...
function setpiercing(piercing)
    ...
    SetPedPropIndex(PlayerPedId(), 2, tonumber(c_options.piercing), 0, 0)
end

function setpiercing_txt(piercing_txt)
    c_options.piercing_txt = piercing_txt
    SetPedPropIndex(PlayerPedId(), 2, tonumber(c_options.piercing), tonumber(c_options.piercing_txt), 0)
end
...
function savehead(head) --Sets Players head in database
    c_options.head = head
    --The laziest way possible of doing this ^_^
    SetPedComponentVariation(PlayerPedId(), 0, tonumber(c_options.head), 0, 2)
    SetPedHeadBlendData(PlayerPedId(), tonumber(c_options.head), tonumber(c_options.head), 0, tonumber(c_options.head), tonumber(c_options.head), 0, 0.5, 0.5, 0.0, false)
end
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--Customisation Menu
...
function savehair(hair) --Sets Players head in database
    c_options.hair = hair
    SetPedComponentVariation(PlayerPedId(), 2, tonumber(c_options.hair), 0, 0)
    HairColour()
end
...
function savecolour(hcolour) --Sets mask texture in database
    ...
    tvRP.setHairColorValue(hcolour)
    --SetPedComponentVariation(PlayerPedId(), 2, tonumber(c_options.hair), tonumber(c_options.hcolour), 2)
    SetPedHairColor(PlayerPedId(), tonumber(c_options.hcolour), tonumber(c_options.hcolour))
    --SetPedComponentVariation(PlayerPedId(), 2, tonumber(c_options.hair), tonumber(c_options.hcolour), 0)
end
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--Customisation Menu
--Male Shirts
...
function saveshirt(shirt) --Sets Players head in database
    ...
    SetPedComponentVariation(PlayerPedId(), 11, 0, 240, 0)
    SetPedComponentVariation(PlayerPedId(), 8, 0, 240, 0)
    SetPedComponentVariation(PlayerPedId(), 11, tonumber(c_options.shirt), 0, 0)
    shirtTextures()
end
...
function saveshirt_txt(shirt_txt) --Sets mask texture in database
    ...
    SetPedComponentVariation(PlayerPedId(), 11, tonumber(c_options.shirt), tonumber(c_options.shirt_txt), 0)
end
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--Customisation Menu
--Hands
...
function savehand(hand) --Sets Players head in database
    c_options.hand = hand
    SetPedComponentVariation(PlayerPedId(), 3, tonumber(c_options.hand), 0, 0)
end
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--Customisation Menu
--Male Shoes
...
function saveshoe(shoe) --Sets Players head in database
    ...
    SetPedComponentVariation(PlayerPedId(), 6, tonumber(c_options.shoes), 0, 0)
    shoeTextures()
end
...
function saveshoe_txt(shoe_txt) --Sets mask texture in database
    ...
    SetPedComponentVariation(PlayerPedId(), 6, tonumber(c_options.shoes), tonumber(c_options.shoes_txt), 0)
end
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--Customisation Menu
--Pants
...
function savepants(pants) --Sets Players head in database
    ...
    SetPedComponentVariation(PlayerPedId(), 4, tonumber(c_options.pants), 0, 0)
    pantsTextures()
end
...
function savepants_txt(pants_txt) --Sets mask texture in database
    ...
    SetPedComponentVariation(PlayerPedId(), 4, tonumber(c_options.pants), tonumber(c_options.pants_txt), 0)
end
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--Customisation Menu
--Undershirt
...
function saveundershirt(undershirt) --Sets Players head in database
    ...
    SetPedComponentVariation(PlayerPedId(), 8, undershirt, 0, 0)
    undershirtTextures()
end
...
function RemoveUnderShirt()
    ...
    SetPedComponentVariation(PlayerPedId(), 8, tonumber(c_options.undershirt), tonumber(c_options.undershirt_txt), 0)
end
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--Customisation Menu
--Undershirt Textures
...
function savearmour(armour) --Sets Players head in database
    ...
    SetPedComponentVariation(PlayerPedId(), 9, tonumber(c_options.armour), 0, 0)
    armourTextures()
end
...
function savearmour_txt(armour_txt) --Sets mask texture in database
    ...
    SetPedComponentVariation(PlayerPedId(), 9, tonumber(c_options.armour), tonumber(c_options.armour_txt), 0)
end
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
--Press E to open/close menu in the red marker
-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
local emplacement = {
    {name="Clothing Store",id=73, x=-275.646,y=6225.750,z=31.696}, -- Paleto Bay
    {name="Clothing Store",id=73, x=1929.848,y=3733.300,z=32.844}, -- Sandy Shores
    {name="Clothing Store",id=73, x=-815.260,y=-187.800,z=37.569}, -- Rockford Hills
    {name="Clothing Store",id=73, x=-34.143,y=-155.807,z=57.076}, -- Hawick
    {name="Clothing Store",id=73, x=-1279.224,y=-1117.125,z=6.990}, -- Vespucci Canals
    {name="Clothing Store",id=73, x=1215.849,y=-473.599,z=66.208}, -- Mirrow Park
    {name="Clothing Store",id=73, x=139.129,y=-1705.272,z=29.292}, -- Davis
    {name="Clothing Store",id=73, x=-1405.869,y=-1021.562,z=23.244} -- Admin Shop
}
incircle = false
    ...
    while true do
        ...
        for k,v in ipairs(emplacement) do
            if(Vdist(pos.x, pos.y, pos.z, v.x, v.y, v.z) < 15.0)then
                DrawMarker(23, v.x, v.y, v.z - 0.97, 0, 0, 0, 0, 0, 0, 0.7, 0.7, 0.5, 1555, 0, 0,165, 0, 0, 0,0)

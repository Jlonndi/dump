...
local function playerConnect(src, deferrals)
...
function table.containts(tbl, value)
...
local function issueQueuePriority(player, deferrals)
    deferrals.update("Checking for Queue Priority Rewards...")

    local discord = GetIdentifier(player, "discord")
    local roles = exports.Badger_Discord_API:GetDiscordRoles(player)
    
    if discord and roles then 
        for _, priority in ipairs(Config.PriorityPermissions) do 
            if table.containts(roles, priority.Role) then
                Queue.AddPriority(discord, priority.Power)
                deferrals.update("Rewarded Queue Priority Power: "..priority.Power.."!")
                Wait(1000)
                break
            end
        end
    else
        deferrals.update("No Queue Priority Found :(")
        Wait(1000)
    end
end

exports.deferralmanager:RegisterHook(2, issueQueuePriority)
exports.deferralmanager:RegisterHook(3, playerConnect)

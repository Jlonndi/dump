...
AddEventHandler('arp_medotry:showText', function(text, source, output, success)
    TriggerOutput(GetPlayerFromServerId(source), text, output, success)
end)

function TriggerOutput(Player, text, action, successpls)
        ...
        while textEnabled do
            ...
            if NetworkIsPlayerActive(Player) then
                ...
                if distance < Config.Distance then
                    local boneCoords = GetPedBoneCoords(GetPlayerPed(Player), 31086, 0, 0, 0)
                    DrawText3D(boneCoords.x, boneCoords.y, boneCoords.z + 0.65 , text, action)
                end
            end
        end
        Displayer = Displayer - 1

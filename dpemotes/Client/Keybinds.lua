if Config.SqlKeybinding then
  ...
  RegisterCommand("emote1bind", function(source, args, rawCommand)
      if not vRP.isHandcuffed({}) and not vRP.isInComa({}) and not IsPedSittingInAnyVehicle(PlayerPedId()) then
        EmoteCommandStart(nil,{emob1, 0})
        Wait(1000)
      end
  end, false)

  RegisterCommand("emote2bind", function(source, args, rawCommand)
      if not vRP.isHandcuffed({}) and not vRP.isInComa({}) and not IsPedSittingInAnyVehicle(PlayerPedId()) then
        EmoteCommandStart(nil,{emob2, 0})
        Wait(1000)
      end
  end, false)

  RegisterCommand("emote3bind", function(source, args, rawCommand)
      if not vRP.isHandcuffed({}) and not vRP.isInComa({}) and not IsPedSittingInAnyVehicle(PlayerPedId()) then
        EmoteCommandStart(nil,{emob3, 0})
        Wait(1000)
      end
  end, false)

  RegisterCommand("emote4bind", function(source, args, rawCommand)
      if not vRP.isHandcuffed({}) and not vRP.isInComa({}) and not IsPedSittingInAnyVehicle(PlayerPedId()) then
        EmoteCommandStart(nil,{emob4, 0})
        Wait(1000)
      end
  end, false)

  RegisterCommand("emote5bind", function(source, args, rawCommand)
      if not vRP.isHandcuffed({}) and not vRP.isInComa({}) and not IsPedSittingInAnyVehicle(PlayerPedId()) then
        EmoteCommandStart(nil,{emob5, 0})
        Wait(1000)
      end
  end, false)

  RegisterCommand("emote6bind", function(source, args, rawCommand)
      if not vRP.isHandcuffed({}) and not vRP.isInComa({}) and not IsPedSittingInAnyVehicle(PlayerPedId()) then
        EmoteCommandStart(nil,{emob6, 0})
        Wait(1000)
      end
  end, false)

  RegisterKeyMapping("emote1bind", "Emote Bind 1", "keyboard", "NUMPAD4");
  RegisterKeyMapping("emote2bind", "Emote Bind 2", "keyboard", "NUMPAD5");
  RegisterKeyMapping("emote3bind", "Emote Bind 3", "keyboard", "NUMPAD6");
  RegisterKeyMapping("emote4bind", "Emote Bind 4", "keyboard", "NUMPAD7");
  RegisterKeyMapping("emote5bind", "Emote Bind 5", "keyboard", "NUMPAD8");
  RegisterKeyMapping("emote6bind", "Emote Bind 6", "keyboard", "NUMPAD9");
  ...
  function EmoteBindStart(args, raw)
      if #args > 0 then
...
RegisterNetEvent("dpemotes:EmoteBindStart")
AddEventHandler("dpemotes:EmoteBindStart", function(args, raw)
    TriggerEvent('gundraw:client:holsterWeapon')
  EmoteBindStart(args, raw)
end)

RegisterNetEvent("dpemotes:EmoteBindsStart")
AddEventHandler("dpemotes:EmoteBindsStart", function()
    TriggerEvent('gundraw:client:holsterWeapon')
  EmoteBindsStart()
end)

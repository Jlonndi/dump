...
function AddEmoteMenu(menu)
    ...
    if Config.SharedEmotesEnabled then
      sharemenu.OnItemSelect = function(sender, item, index)
          ...
          TriggerEvent('vrp:client:selectTargetPlayer', 2, function(selectedPed)
              if selectedPed ~= -1 then
                  _,_,rename = table.unpack(DP.Shared[ShareTable[index]])
                  TriggerServerEvent("ServerEmoteRequest", GetPlayerServerId(selectedPed), ShareTable[index])
              end
          end)
        end
      end
      ...
      shareddancemenu.OnItemSelect = function(sender, item, index)
          TriggerEvent('vrp:client:selectTargetPlayer', 2, function(selectedPed)
              if selectedPed ~= -1 then
                  ...
                  TriggerServerEvent("ServerEmoteRequest", GetPlayerServerId(selectedPed), DanceTable[index], 'Dances')
              end
          end)
      end
    end

...
Citizen.CreateThread(function()
  while true do
    ...
    if Config.MenuKeybindEnabled and IsControlPressed(0, Config.MenuKeybind) then
      if not vRP.isHandcuffed({}) and not vRP.isInComa({}) and not vRP.getActionLock({}) then
        OpenEmoteMenu()
      end
    end
    if Config.EnableXtoCancel and IsControlPressed(0, 246) then
      if not vRP.isHandcuffed({}) and not vRP.isInComa({}) and not vRP.getActionLock({}) then
        EmoteCancel()
      end
    end
...
function EmoteCommandStart(source, args, raw, force)
    local cancelled = false

    TriggerEvent('core:client:isInterfaceOpened', function(ui_open)
      cancelled = ui_open
    end)

    if cancelled and not force then
      return
    end
...
RegisterNetEvent("dpemotes:clearAll")
AddEventHandler("dpemotes:clearAll", function()
  EmoteCancel()
  _menuPool:CloseAllMenus()
end)

RegisterNetEvent("dpemotes:EmoteCommandStart")
AddEventHandler("dpemotes:EmoteCommandStart", function(args, raw)
  if not vRP.isHandcuffed({}) and not vRP.isInComa({}) and not vRP.getActionLock({}) then
    EmoteCommandStart("", {args, 0}, raw, true)
  end
end)

...
-- Slower, raycasting thread
Citizen.CreateThread(function()
    while true do 
        local plyPos = GetEntityCoords(PlayerPedId())
        vehicleInFront = VehicleInFront(plyPos)
        Wait(500)
    end
end)
...
Citizen.CreateThread(function()
    while true do
        if vehicleInFront then
            local trunk = GetEntityBoneIndexByName(vehicleInFront, 'boot')
            if trunk ~= -1 then
                local coords = GetWorldPositionOfEntityBone(vehicleInFront, trunk)
                if #(GetEntityCoords(PlayerPedId()) - coords) <= 1.5 then
                    if not inTrunk then
                        if GetVehicleDoorAngleRatio(vehicleInFront, 5) < 0.9 then
                            DrawText3D(coords.x, coords.y, coords.z, '[~g~H~w~] Open | [~r~K~w~] Hide')
                            if IsControlJustReleased(0, 74)then
                                SetCarBootOpen(vehicleInFront)
                            end
                        else
                            DrawText3D(coords.x, coords.y, coords.z, '[~g~H~w~] Close | [~r~K~w~] Hide')
                            if IsControlJustReleased(0, 74) then
                                SetVehicleDoorShut(vehicleInFront, 5)
                            end
                        end
                    end
                    if IsControlJustReleased(0, 311) and not inTrunk then
                        getInTrunk(vehicleInFront)
                    end
                end
            end
        end
...
function trunkCam()
    ...
    AttachCamToEntity(cam, ped, 0.0, -2.5, 1.0, true)
...
function VehicleInFront()
    ...
    local rayHandle = StartShapeTestLosProbe(pos.x, pos.y, pos.z, entityWorld.x, entityWorld.y, entityWorld.z, 10, plyPed, 0)

...
local street_overlay = false

AddEventHandler('helicam:client:toggleStreetOverlay', function()
	street_overlay = not street_overlay
end)

local cam_last_coords = nil
local cam_last_target = nil
local cam_last_rot = nil
local a_length = nil
local b_length = nil
local z_length = nil
local nodes = {}

-- The complicated math below can be visualised with the following diagram: https://i.imgur.com/RPAIQAi.png
Citizen.CreateThread(function()
	while true do
		Citizen.Wait(100)

		if cam_last_coords and street_overlay then
			local _, groundZ = GetGroundZFor_3dCoord(cam_last_coords.x, cam_last_coords.y, cam_last_coords.z, false)

			z_length = cam_last_coords.z - groundZ

			local rotation_elevation = cam_last_rot.x
			local rotation_around_z = cam_last_rot.z

			if rotation_around_z < 0 then
				rotation_around_z = math.abs(rotation_around_z)
			else
				rotation_around_z = 360 - rotation_around_z
			end

			rotation_around_z = math.ceil(rotation_around_z)

			if rotation_elevation < 0 then
				rotation_elevation = math.ceil(math.abs(rotation_elevation))

				local direction = RotationToDirection(vector3(rotation_elevation, 0, rotation_around_z))

				a_length = math.tan(math.rad(90 - rotation_elevation)) * z_length
				b_length = math.sqrt((a_length * a_length) + (z_length * z_length))

				local new_location = cam_last_coords + (direction * b_length * vector3(-1, 1, 1))
				new_location = vector3(new_location.x, new_location.y, groundZ)

				if #(new_location.xy - cam_last_coords.xy) > 1000 then
					cam_last_target = nil
					nodes = {}
				else
					if not cam_last_target or #(cam_last_target.xy - new_location.xy) > 75 then
						cam_last_target = new_location

						local new_nodes = {}

						local origin_coords = new_location

						local cached_positions = {}

						for i = 1, 20, 1 do
							--local retval, node_coords, heading = GetNthClosestVehicleNodeWithHeading(origin_coords.x, origin_coords.y, origin_coords.z, i * 10)
							local retval, node_coords = GetNthClosestVehicleNode(origin_coords.x, origin_coords.y, origin_coords.z, i * 10)

							local too_close = false

							for _, cached_position in ipairs(cached_positions) do
								if #(node_coords - cached_position) < 75 then
									too_close = true
								end
							end

							if not too_close then
								table.insert(cached_positions, node_coords)
...
Citizen.CreateThread(function()
	while true do
		...
		if helicam then
			if street_overlay then
				drawTxt(0.85, 1.13, 0.4, 0.4, 0.3, 'STREET', 55, 155, 55, 255)

				for _, node in ipairs(nodes) do
					if cam_last_target ~= vector3(0, 0, 0) then
						local x, y, z = World3dToScreen2d(node.pos.x, node.pos.y, node.pos.z)

						local street_name, _ = GetStreetNameAtCoord(node.pos.x, node.pos.y, node.pos.z)
						...
						-- Draw text
						AddTextComponentString(street_name)
						BeginTextCommandDisplayText('STRING')
						SetTextCentre(1)
						EndTextCommandDisplayText(y, z)
					end
				end
			else
				drawTxt(0.85, 1.13, 0.4, 0.4, 0.3, 'STREET', 255, 255, 255, 255)
			end
		end
	end
end)
	...
	while true do
		...
		if helicam then
			...
			while helicam and not IsEntityDead(lPed) and (GetVehiclePedIsIn(lPed) == heli) and IsHeliHighEnough(heli) do
				...
				cam_last_coords = GetCamCoord(cam)
				cam_last_rot = GetCamRot(cam)
...
function IsPlayerInPolmav()
	...
	local inBird = false
	if IsVehicleModel(vehicle, polmav_hash) or IsVehicleModel(vehicle, polmav2_hash) then
		inBird = true
	end
	return inBird
end
